...
if [ $# -lt 2 ]; then
    echo "usage: $0 <subtitle_path> <lang_code2>" >&2
    exit 2
fi

DIR=$(dirname "$0")

python3 "$DIR/ai-translate-sub.py" "$1" "$2" || echo "ai-translate-sub: failed (continuing)" >&2
python3 "$DIR/merge-subs.py" "$1" "$2" || echo "merge-subs: failed (continuing)" >&2
